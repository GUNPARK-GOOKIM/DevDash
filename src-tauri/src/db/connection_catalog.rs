//! Connection catalog shared by Desktop, CLI, and Mobile.
//! Metadata on disk (`connections.json`); passwords stay in the OS keyring (`devdash_app`).
use crate::db::credentials;
use crate::db::paths::{connections_path, ensure_config_dir};
...
pub struct CatalogConnection {
    ...
    /// RFC3339 last local edit. Empty on pre-sync catalogs (treated as oldest).
    #[serde(default)]
    pub updated_at: String,
    /// Device that last wrote this record (for conflict traces).
    #[serde(default)]
...
impl CatalogConnection {
    pub fn touch(&mut self, device_id: &str) {
        ...
        if !device_id.is_empty() {
            self.origin_device_id = device_id.to_string();
        }
    }
...
pub fn load_catalog_from_path(path: &Path) -> Result<ConnectionCatalog, String> {
    ...
    serde_json::from_str(&raw)
        .map_err(|e| format!("Invalid connection catalog {}: {e}", path.display()))
}
...
pub fn save_catalog_to_path(path: &Path, catalog: &ConnectionCatalog) -> Result<(), String> {
    ...
    let json = serde_json::to_string_pretty(catalog).map_err(|e| format!("Serialize catalog: {e}"))?;
...
pub fn resolve_connection<'a>(
    ...
    let matches: Vec<_> = catalog
        ...
        .iter()
        .filter(|c| {
            c.name.to_lowercase().starts_with(&needle.to_lowercase()) || c.id.starts_with(needle)
        })
        .collect();
    match matches.len() {
        1 => Ok(matches[0]),
...
pub fn default_connection<'a>(
    ...
    Err("No default connection. Run `devdash connect use <name>` or pass `-c <name>`.".into())
}

pub fn upsert_connection(catalog: &mut ConnectionCatalog, mut conn: CatalogConnection) {
    if conn.updated_at.trim().is_empty() {
...
pub fn remove_connection(
    ...
    name_or_id: &str,
) -> Result<CatalogConnection, String> {
...
mod tests {
    ...
    #[test]
    fn legacy_catalog_without_sync_fields_loads() {
        let path = temp_catalog_path();
        fs::write(
            &path,
            r#"{
        ...
        .unwrap();
        let loaded = load_catalog_from_path(&path).unwrap();
        assert_eq!(loaded.connections[0].updated_at, "");
        assert_eq!(loaded.connections[0].origin_device_id, "");
        let _ = fs::remove_file(path);
    }
}
